...
SpriteBatch::SpriteBatch()
{
	...
	_Camera = NULL;
	d3ddevice = CDevice::getInstance()->getD3DDevice();
}
...
void SpriteBatch::Create()
{
	...
	if (res != D3D_OK) {
		Trace::Log("Fail to create SpriteHandler");
		return;
	}
...
void  SpriteBatch::GetActualPosition(D3DXVECTOR3 * postion)
{
	...
	_CameraMatrix._22 = -1;
	_CameraMatrix._41 = -(_Camera->getPosition().x /*- SCREEN_WIDTH / 2.0*/);
	_CameraMatrix._42 = +_Camera->getPosition().y /*+ SCREEN_HEIGHT / 2.0*/;
...
bool SpriteBatch::IsOutsideCam(float x, float y, float width, float height)
{
	...
	if (_Camera->getPosition().x + SCREEN_WIDTH <= x - width / 2 || _Camera->getPosition().x - SCREEN_WIDTH >= x + width / 2
		|| _Camera->getPosition().y + SCREEN_HEIGHT <= y - height / 2 || _Camera->getPosition().y - SCREEN_HEIGHT >= y + height / 2)
	{
		return true;
	}
